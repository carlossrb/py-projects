    ...
    while sair == False:
        ...
        for velas in data:
            ...
            if horario not in analise:
                analise.update(
                    {horario: {'verde': 0, 'vermelha': 0, 'doji': 0, 'dir': '.'}})
            ...
            for i in range(int(martingale)):
                ...
                if mg_time in catalogacao[par]:
                    ...
                    catalogacao[par][horario]['mg' +
                                              str(i)]['verde'] += catalogacao[par][mg_time]['verde'] + soma['verde']
                    catalogacao[par][horario]['mg' + str(
                        i)]['vermelha'] += catalogacao[par][mg_time]['verde'] + soma['vermelha']
                    catalogacao[par][horario]['mg' +
                                              str(i)]['doji'] += catalogacao[par][mg_time]['verde'] + soma['doji']

                    catalogacao[par][horario]['mg' + str(i)]['%'] = round(100 * (catalogacao[par][horario]['mg' + str(i)]
                                                                                 ['verde' if catalogacao[par][horario]['dir'] == 'CALL' else 'vermelha'] / (catalogacao[par][horario]['mg' + str(i)]['verde'] +
                                                                                                                                                            catalogacao[par][horario]['mg' + str(i)]['vermelha'] + (catalogacao[par][horario]['mg' + str(i)]['doji']))))
...
for par in catalogacao:
    for horario in sorted(catalogacao[par]):
            ...
            if martingale.strip() != '':
                for i in range(int(martingale)):
                    if catalogacao[par][horario]['mg' + str(i + 1)]['%'] >= porcentagem:
                        ...
                        break
        if ok == True:
            temp = Fore.GREEN if catalogacao[par][horario]['dir'] == 'CALL' else Fore.RED
            msg = Fore.YELLOW + par + Fore.RESET + ' - ' + horario + ' - ' + temp + catalogacao[par][horario]['dir'] + Fore.RESET + ' - ' + str(catalogacao[par][horario]['%']) + '% - ' + Back.GREEN + Fore.BLACK + str(
                catalogacao[par][horario]['verde']) + Back.RED + str(catalogacao[par][horario]['vermelha']) + Back.RESET + Fore.RESET + str(catalogacao[par][horario]['doji'])

        if martingale.strip() != ' ':
            for i in range(int(martingale)):
                i += 1
                if str(catalogacao[par][horario]['mg' + str(i)]['%']) != 'N\A':
                    msg += '  | MG ' + str(i) + ' - ' + str(catalogacao[par][horario]['mg' + str(i)]['%']) + ' % - ' + Back.GREEN + Fore.BLACK + str(catalogacao[par][horario]['mg' + str(
        ...
        print(msg)
        open('sinais_' + (datetime.now()). strftime('%Y-%m-%d') + ' ' + str(timeframe) + 'M.txt',
             'a').write(horario + ',' + par + ',' + catalogacao[par][horario]['dir'].strip())
